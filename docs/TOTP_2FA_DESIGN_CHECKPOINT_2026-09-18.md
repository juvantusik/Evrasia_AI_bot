...
4. native Bitrix pending-enrollment and QR frontend contracts are confirmed, and the pilot QR/TOTP continuation is now deployed in production. `security.user.otp.init` semantics are reused: `regenerate()` in memory, built-in `qrcode`/`QRCode` at 164x164 level H, then the same secret plus one 6-digit authenticator code posted back for `regenerate($binarySecret)->syncParameters(...)->save()`. Production SHAs after deployment: account template `222b3c19d46da4b127e79b37a96c3d3676f8ba0b0bf84a26ec9f3b71cd21de41`, account JS `352447d463aa0dcea91abfae1d0690682424343bf6fbdb9dc8428dee9b6cb17a`, TOTP enrollment component `4a1bfbd277980dd529f89b43bc834498a543de3f371df4870963a3a4e6b09833`. Deployment verification kept `OTP_ENABLED=NO`, `OTP_MANDATORY=NO`, `PILOT_OTP_ROWS=0`, with no session revocation or PIN-flow change. Next gate is the browser QR scan + TOTP confirmation for USER_ID `880339`; only after the new OTP row is verified should global OTP/session revocation be performed separately under guards;
