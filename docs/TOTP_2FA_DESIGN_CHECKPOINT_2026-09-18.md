...
4. native Bitrix pending-enrollment and QR frontend contracts are now confirmed. `security.user.otp.init` calls `regenerate()` in memory, returns `SECRET` / `PROVISION_URI` to the browser, renders the QR with the built-in `qrcode` extension (`QRCode`, 164x164, level H), and posts the same secret plus the authenticator code back for `regenerate($binarySecret)->syncParameters(...)->save()`. TOTP requires one 6-digit code. There is no native pending-secret DB row before verification. `getDeferredParams()` is public static. Do not invent a parallel pending-secret database or QR library. The pilot remains `OTP_ENABLED=NO`, `PILOT_OTP_ROWS=0`. Next implement the pilot QR/TOTP step behind the already-passed SMS session gate; after successful TOTP sync/save, verify the new row, then separately enable native global OTP in optional/non-mandatory mode and revoke pre-2FA sessions under guards;
