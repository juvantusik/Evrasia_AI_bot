...
4. native Bitrix pending-enrollment semantics are now confirmed: `security.user.otp.init` calls `regenerate()` in memory, exposes the generated `SECRET` / `PROVISION_URI` to the browser, then the confirmation request posts the same secret back and the server runs `regenerate($binarySecret)->syncParameters(...)->save()`. There is no native pending-secret DB row before verification. `getDeferredParams()` is public static. Do not invent a parallel pending-secret database. Next inspect/reuse the native QR/frontend rendering contract, then implement the pilot QR step behind the already-passed SMS session gate; after successful TOTP sync/save, revoke pre-2FA sessions and enable native global OTP in optional/non-mandatory mode under guards;
